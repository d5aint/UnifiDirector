...
from ...models.integration_wifi_broadcast_page_dto import IntegrationWifiBroadcastPageDto
...
) -> IntegrationWifiBroadcastPageDto | None:
    if response.status_code == 200:
        response_200 = IntegrationWifiBroadcastPageDto.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[IntegrationWifiBroadcastPageDto]:
    return Response(
...
def sync_detailed(
    ...
    *,
    ...
    |`broadcastingFrequenciesGHz`|`SET(DECIMAL)`|`contains` `containsAny` `containsAll`
    `containsExactly`|
    ...
    |`broadcastingDeviceFilter.deviceIds`|`SET(UUID)`|`contains` `containsAny` `containsAll`
    `containsExactly`|
    |`broadcastingDeviceFilter.deviceTagIds`|`SET(UUID)`|`contains` `containsAny` `containsAll`
    `containsExactly`|
